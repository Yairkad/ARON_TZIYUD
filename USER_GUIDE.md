...
#### 🎯 בחירת ציוד מרובה (תכונה חדשה!)
- **ניתן לבחור מספר פריטים בהשאלה אחת**
- סמן את כל הפריטים שאתה צריך
- המערכת תעדכן את המלאי של כל הפריטים אוטומטית

#### ⚠️ שים לב:
- אם אין ציוד זמין, הכפתור יהיה כבוי
- ניתן לבחור כמה פריטים שתרצה (במידה והם זמינים)
...
### 🎨 תצוגת קטגוריות עם תמונות
- **סרגל קטגוריות דביק** - נשאר למעלה גם בגלילה
- לכל קטגוריה אייקון ייעודי
- **מיון אוטומטי לפי שימושיות** - ציוד פופולרי מופיע ראשון
- **סינון לפי קטגוריה** - לחץ על קטגוריה לראות רק את הפריטים שלה
- טאב "הכל" להצגת כל הציוד

### 📈 רשת כרטיסי המלאי
- כל פריט ציוד בכרטיס נפרד עם תמונה
- **ציוד זמין (ירוק):** ✅ + שם הציוד + תמונה
- **ציוד לא זמין (אפור):** ❌ + "חסר זמנית"
- עדכון בזמן אמת אחרי כל השאלה/החזרה
- ציוד שנבחר עולה לראש הרשימה
...
**שדות אופציונליים:**
- 🖼️ **תמונה מ-Google Drive** - הוסף קישור לתמונה (מומלץ!)
  - העלה תמונה ל-Google Drive
  - שתף אותה ("כל מי שיש לו קישור יכול לצפות")
  - העתק את הקישור והמר אותו באמצעות כלי ההמרה
- ✓ **ציוד מתכלה** - סמן אם זה ציוד חד-פעמי

**תהליך:**
1. מילוי פרטי הציוד
2. אופציונלי: הוספת קישור לתמונה
3. לחיצה על **"הוסף ציוד"**
4. הציוד מתווסף מיד למערכת
...
- שינוי סטטוס ציוד מתכלה
- **🖼️ הוספה/עדכון תמונה** - הוסף או שנה קישור לתמונה מ-Google Drive

**תהליך עריכה:**
1. לחיצה על **"ערוך"** ליד הפריט
2. שינוי הפרטים (שם, קטגוריה, כמות, תמונה)
3. **שדה תמונה** - מופיע בשורה מלאה מתחת לשאר השדות
4. **"שמור"** - לאישור
5. **"ביטול"** - לביטול השינויים
...
- **חישוב חכם** - המערכת מחשבת אוטומטית איזה ציוד מושאל הכי הרבה
- **עדכון יומי** - כל לילה ב-3:00 המערכת מעדכנת את סדר התצוגה
- **מיון לפי עיר** - כל עיר מקבלת מיון משלה
- **מבוסס על 6 חודשים אחרונים** - מתחשב רק בהשאלות האחרונות
- ציוד פופולרי מוצג ראשון, חוסך זמן חיפוש!

📖 **מידע נוסף**: ראה `PRIORITY_CALCULATION.md` בפרויקט

### 🖼️ תמונות מ-Google Drive
- **חיסכון במקום** - תמונות מאוחסנות ב-Google Drive (חינם!)
- **קל לעדכון** - שנה תמונה בדרייב והקישור נשאר זהה
- **כלי המרה** - השתמש בכלי ההמרה ב-`tools/google-drive-url-converter.html`

📖 **מידע נוסף**: ראה `tools/README.md`
...
### 🖼️ איך מוסיפים תמונה לציוד?
1. העלה את התמונה ל-Google Drive
2. לחץ ימין על התמונה → "שתף"
3. בחר "כל מי שיש לו קישור יכול לצפות"
4. העתק את הקישור
5. פתח את `tools/google-drive-url-converter.html` בדפדפן
6. הדבק את הקישור והמר אותו לקישור ישיר
7. העתק את הקישור הישיר והדבק בשדה "תמונה" בניהול הציוד
...
## 📦 תכונות חדשות בגרסה 2.5.0

✨ **מיון אוטומטי לפי שימושיות** - ציוד פופולרי מופיע ראשון
✨ **תמונות מ-Google Drive** - הוסף תמונות לציוד בקלות
✨ **סינון לפי קטגוריה** - סנן ציוד לפי קטגוריה ספציפית
✨ **בחירת ציוד מרובה** - בחר מספר פריטים בהשאלה אחת
✨ **כלי המרת קישורים** - המר קישורי Google Drive לתצוגה ישירה

📖 לפרטים מלאים ראה `CHANGELOG.md`
